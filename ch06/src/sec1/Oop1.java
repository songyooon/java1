...
public class Oop1 {
	...
	public static void main(String[] args) {
		...
		//인스턴스(객체)는 자바에서 기본으로 제공하는 클래스이거나  2)사용자가 필요에 따라 만든 클래스로부터
		//생성될 수 있다.
		...
		//상속 후에 메서드나 생성자 함수를 여러가지 형태로 다르게 바꿀 수 있는데 이를 메서드 오버라이딩과 - 상속을 해준 상태에서 바꾸는 것 
		//하나의 클래스 내에서 메서드나 함수의 매개변수의 개수나 변수데이터타입에 따라 같은 이름으로 여러 개 지정이 가능한 데 오버로딩 - 하나의 내에서 여러개가 존재하는 것 (오버라이딩/오버로딩 차이 알아야함)
		//객체를 만들 때 여러 형태로 만들거나 선언할 수도 있고, 
		//메서드 오버라이딩이나 메서드 오버로딩과 같이 같은 이름의 여러 형태로 활용할 수 있음 -> 다형성
		//추상화 - 간소화된 꼭 필요한 요소만으로 구성하여 다른 서브 클래스들에서 공통으로 활용할 수 있도록 
		//상세한 내용을 기술하지 않고 선언만 하는것    -> 상속받을 재산보다 빚이 더 많으면 상속포기하는 것처럼
		//-> 일부만 추상화된 클래스(추상화 클래스), 완전한 추상화 클래스(인터페이스)
		//객체지향의 특징 1 캡슐화 2 정보은닉 3 상속 4 다형성 5 추상화
	}
